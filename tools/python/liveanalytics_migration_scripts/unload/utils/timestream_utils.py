...
class TimestreamUtility:
    ...
    def timestream_unload(
        ...
        for index, start_end_pair in enumerate(batches, start=1):
            ...
            self.log_unload(
                ...
                table=table,
                migration_tag=migration_tag,
                time_range=f"start_time >= {batch_start_time} and end_time < {batch_end_time}",
                status=f"batch{index}_started",
            )
            rows_exported = self.run_unload_query(
                query,
                database,
                table,
                batch_start_time,
            ...
            self.log_unload(
                ...
                status=f"batch{index}_completed",
            )
    ...
    def run_unload_query(
        ...
        try:
            ...
            manifest_file_response = self.s3_utility.fetch_json_from_s3(
                s3_manifest_bucket_name, file
            )
            self.logger.debug(manifest_file_response)
            exported_rows = manifest_file_response["query_metadata"]["total_row_count"]
            self.logger.info(
                f"Rows exported for batch start_time >= {start_time} and end_time < {end_time} for {database}.{table}: {exported_rows}"
            )
