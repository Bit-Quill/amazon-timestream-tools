...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r",
        "--region",
        help="AWS region of your Timestream table to be unloaded",
        default=None,
        required=False,
    )
    parser.add_argument(
        "-d", "--database", help="Timestream database name", required=False
    )
    parser.add_argument(
        "-t", "--table", help="Timestream table name to be unloaded", required=False
    )
    parser.add_argument(
        "-s",
        "--s3-uri",
        help="S3 Bucket URI to store unload data. Example s3://abc",
        required=False,
    )
    parser.add_argument(
        "-f",
        "--start-time",
        help="UTC start timestamp for data unload.Format: 'YYYY-MM-DD HH:MM:SS'Example: '2024-03-26 17:24:38' Note: Must be in UTC timezone",
        required=True,
    )
    parser.add_argument(
        "-e",
        "--end-time",
        help="UTC end timestamp for data unload.Format: 'YYYY-MM-DD HH:MM:SS'Example: '2025-03-26 17:24:38' Note: Must be in UTC timezone",
        required=False,
    )
    parser.add_argument(
        "-p",
        "--partition",
        help="Partition data by 'hour','day', 'month' or 'year'",
        required=False,
    ...
    parser.add_argument(
        "-ed",
        "--export-database",
        help="Whether to export all tables within a database",
        default=False,
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "-et",
        "--export-table",
        help="Whether to export a table",
        default=False,
        required=False,
        action="store_true",
    )
    parser.add_argument(
        ...
        help="Whether to export all tables within all databases within an AWS region",
        default=False,
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "-sns",
        "--sns-topic-arn",
        help="SNS topic ARN for sending any batchload failures",
        default=None,
        required=False,
    )
    parser.add_argument(
        "-edl",
        "--enable-dynamodb-logger",
        help="Whether to enable logging of export progress in DynamoDB",
        default=False,
        required=False,
        action="store_true"
    )
    parser.add_argument(
        "-mt",
        "--migration-tag",
        help="Migration tag-used as sort key to store DynamoDB",
        default=None,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-c",
        "--compression",
        help="Compress the export files",
        default="NONE",
        choices=["NONE", "GZIP"],
        required=False,
    )
    parser.add_argument(
        "-ms",
        "--max-file-size",
        help="Max individual file size in GB or MB for unload",
        default="78GB",
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--field-delimiter",
        default=",",
        help="Character used to separate fields in CSV files (default: comma)",
    )
    parser.add_argument(
        "-ik",
        "--kms-key",
        help="KMS key to be used to encrypt the data in S3",
        default=None,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--recent-first",
        default=False,
        help="Set to true to load data in reverse chronological order (most recent batch first)",
        required=False,
        action="store_true"
    )
    ...
    if not (args.export_database or args.export_table or args.export_all_databases):
        raise ValueError(
            "At least one of --export-database, --export-table, or --export-all-databases must be specified."
        )
    else:
        unload_type = (
            "database"
    ...
    if args.sns_topic_arn is not None:
        ...
        assert sns_region == region, (
            f"The specified SNS topic ARN does not match the provided region. {region}"
        )
    ...
    timestream_utility = TimestreamUtility(
        region, sns_topic_arn, args.enable_dynamodb_logger
    )
