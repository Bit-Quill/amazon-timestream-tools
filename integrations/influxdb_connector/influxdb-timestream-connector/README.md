...
| `KMSKey`  | AWS KMS key for the database. If the KMS key is not specified, the database will be encrypted with a Timestream managed KMS key located in your account. | |
| `Tags`  | JSON string of key-value pairs to label the database. | |
...
4. Configure the following environment variables:
    ...
    - `kms_key_id` string: AWS KMS key for the database.
    - `tags` string: JSON string of key-value pairs to label the database.
