...
DS_RESPONSE=$(curl -s -X POST "${GRAFANA_URL}/api/datasources" \
  ...
  -d "{
    ...
    \"basicAuth\": false,
    \"jsonData\": {
      \"defaultBucket\": \"${INFLUXDB_V2_BUCKET:-bucket}\",
...
echo "Updating dashboard with new datasource UID..."
UPDATED_DASHBOARD=$(cat "$DASHBOARD_FILE" | sed "s/my_dashboard_id/$NEW_DS_UID/g")
...
DASHBOARD_PAYLOAD=$(echo "$UPDATED_DASHBOARD" | jq '{
  ...
  folderId: 0,
  overwrite: true,
  message: "Imported via API with updated datasource UID"
}')
