...
def main():
    parser = argparse.ArgumentParser(description='Backup or restore Timestream for InfluxDB data')
    parser.add_argument('--operation', choices=['backup', 'restore'], 
                        required=True, help='Operation to perform (backup or restore)')
    parser.add_argument('--unique-restore-name', action='store_true',
    ...
    try:
        ...
        logger.info(f"Successfully retrieved tokens from Secrets Manager: {tokens_secret_name}")
    except Exception as e:
...
def backup(session, backup_endpoint, backup_token, s3_bucket_name, bucket_name, org_name):
    ...
    try:
        ...
        safe_error = str(e.stderr).replace(backup_token, "****") if backup_token else str(e.stderr)
    ...
    logger.info(f"Backup complete. Uploaded {file_count} files ({total_size_mb:.2f} MB) to S3")
...
def restore(session, restore_endpoint, restore_token, s3_bucket_name, bucket_name, org_name,
    ...
    latest_prefix = sorted(response['CommonPrefixes'], key=lambda x: x['Prefix'])[-1]['Prefix']
    ...
    if force_replace:
        ...
        logger.info(f"Checking if bucket {target_bucket_name} exists at {restore_endpoint}")
        ...
        try:
            ...
            if "bucket not found" in e.stderr.lower():
                logger.info(f"Bucket {target_bucket_name} does not exist, no need to delete")
            else:
                logger.warning(f"Error during bucket deletion: {e.stderr}")
        ...
        raise RuntimeError("Restore failed: Neither force_replace nor unique_restore_name were set")
    ...
    try:
        ...
        safe_error = str(e.stderr).replace(restore_token, "****") if restore_token else str(e.stderr)
